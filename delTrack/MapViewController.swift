...
class MapViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
    ...
    //            latitude = lelandLatitude + (universityCityLibraryLatitude-lelandLatitude)*seconds/60
            ...
            if ((minutes == 36 && seconds > stop_time) || minutes == 37 || minutes == 38 || minutes == 39){
                ...
                if (seconds_delay >= Double(120-stop_time)){
                    curve_adjustment_enter = 0.0007789382754*sin(acos((seconds_delay-120) / -60.0 + 1)) * pow(sin(acos((seconds_delay-120) / -60.0 + 1)/2),1.5)
                }
    ...
    //            print(seconds_delay)
    //            print(seconds_ratio)
                location = [delmarAndDebalivereLatitude + (cornerLatitude-delmarAndDebalivereLatitude)*Double(seconds_ratio), delmarAndDebalivereLongitude + (cornerLongitude-delmarAndDebalivereLongitude)*Double(seconds_ratio)]
            }
    ...
    //            print(seconds_delay)
    //            print(seconds_ratio)
                location = [cornerLatitude + (hamiltonAvenueLatitude-cornerLatitude)*Double(seconds_ratio), cornerLongitude + (hamiltonAvenueLongitude-cornerLongitude)*Double(seconds_ratio)]
            }
